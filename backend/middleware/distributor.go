	...
	return func(c *gin.Context) {
		...
		if ok {
			...
			if shouldSelectChannel {
				...
				// Playground：任意 /pg/* 均可带 model/group，与 /v1 等价选路
...
func getModelRequest(c *gin.Context) (*ModelRequest, bool, error) {
	...
	// 在线搜索：客户端可不传 model，与 dto.DefaultSearchRoutingModel 对齐供选路
	if strings.HasPrefix(c.Request.URL.Path, "/v1/search") && modelRequest.Model == "" {
		modelRequest.Model = dto.DefaultSearchRoutingModel
	}
