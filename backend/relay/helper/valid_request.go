...
func GetAndValidateSearchRequest(c *gin.Context) (*dto.SearchRequest, error) {
	...
	if strings.TrimSpace(searchRequest.Model) == "" {
		searchRequest.Model = dto.DefaultSearchRoutingModel
		if searchRequest.RawParams != nil {
			searchRequest.RawParams["model"] = searchRequest.Model
		}
	}
