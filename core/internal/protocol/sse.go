...
// replayWireSSEExtension re-emits a preserved SSE record from an extension node.
func replayWireSSEExtension(ext *ExtensionNode) ([][]byte, error) {
	if ext == nil {
		return nil, nil
	}
	var wire WireSSEPayload
	if err := json.Unmarshal(ext.Payload, &wire); err != nil {
		return nil, err
	}
