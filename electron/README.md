...
- **本地代理**：Electron 在主进程调用 **Go `clovapi proxy start`**（与 CLI 同源，后台 daemon），监听 `profiles.json` 中的 `proxy.host` / `proxy.port`（默认 `http://127.0.0.1:27483`）。请求路由与 Go 内核一致：`http://127.0.0.1:{port}/{providerId}/v1/…`，由 `/messages`、`/responses`、`/chat/completions` 等 endpoint 区分 API style，模型从请求体 `model` 读取。
...
`release-switcher` (`v*` tags) covers core CLI / npm only — not the Electron shell.
