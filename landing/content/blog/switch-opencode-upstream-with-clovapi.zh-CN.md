...
`add` 阶段选择或确认 API 形态（Anthropic 兼容、OpenAI 兼容、Gemini 等），`switch` 会把选中的 vendor/model 绑定映射到 OpenCode 的 provider 结构。多个模型可并存，切换只需改绑定。
...
`clovapi list` 会展示各 CLI 当前绑定的 vendor/model 与 API 形态矩阵。
