...
const SetupWizard = () => {
  ...
  const onSubmit = () => {
    ...
    API.post('/api/setup', formValues)
      .then((res) => {
        ...
        if (success) {
          showNotice("系统初始化成功，正在跳转...");
          setTimeout(() => {
            window.location.reload();
          }, 1500);
        } else {
          showError(message || "初始化失败，请重试");
        }
      })
      .catch((error) => {
  ...
  const getStepContent = (step) => {
    switch (step) {
      case 0:
        return <DatabaseStep setupStatus={setupStatus} />;
      case 1:
        return (
          <AdminStep
      ...
      case 3:
        return (
          <CompleteStep setupStatus={setupStatus} formData={formData} />
        );
      default:
        return null;
    }
    ...
    <div className='min-h-screen flex items-center justify-center px-4'>
      <div className='w-full max-w-4xl'>
        <Card className='!rounded-2xl shadow-sm border-0'>
          <div className='mb-4'>
            <div className='text-xl font-semibold'>{"系统初始化"}</div>
