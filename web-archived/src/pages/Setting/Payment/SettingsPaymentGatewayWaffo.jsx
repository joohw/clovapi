...
export default function SettingsPaymentGatewayWaffo(props) {
  ...
  const handleIconFileChange = (e) => {
    ...
    if (file.size > MAX_ICON_SIZE) {
      showError("图标文件不能超过 100KB，请压缩后重新上传");
      e.target.value = '';
      return;
    }
    ...
    <Spin spinning={loading}>
      ...
      >
        <Form.Section text={"Waffo 设置"}>
          ...
          <Banner
            type='info'
            description={"请在 Waffo 后台获取 API 密钥、商户 ID 以及 RSA 密钥对，并配置回调地址。"}
          />
          ...
          <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 24, xl: 24, xxl: 24 }}>
            ...
            <Col xs={24} sm={24} md={8} lg={8} xl={8}>
              <Form.InputNumber
                ...
                min={0}
                step={0.1}
                extraText={"每个充值单位对应的 USD 金额，默认 1.0"}
              />
            </Col>
            <Col xs={24} sm={24} md={8} lg={8} xl={8}>
              <Form.InputNumber
            ...
            <Col xs={24} sm={24} md={12} lg={12} xl={12}>
              <Form.Input
                ...
                label={"回调通知地址"}
                placeholder={"例如 https://example.com/api/waffo/webhook"}
                extraText={"留空则自动使用 服务器地址 + /api/waffo/webhook"}
              />
            </Col>
            <Col xs={24} sm={24} md={12} lg={12} xl={12}>
              <Form.Input
                ...
                label={"支付返回地址"}
                placeholder={"例如 https://example.com/console/topup"}
                extraText={"支付完成后用户跳转的页面，留空则自动使用 服务器地址 + /console/topup"}
              />
            </Col>
          </Row>
      ...
      <div style={{ marginTop: 24 }}>
        <Typography.Title heading={6} style={{ marginBottom: 8 }}>{"支付方式"}</Typography.Title>
        <Text type='secondary'>
        ...
        <Table
          ...
          pagination={false}
          size='small'
          empty={<Text type='tertiary'>{"暂无支付方式，点击上方按钮新增"}</Text>}
        />
        <Button onClick={submitWaffoSetting} style={{ marginTop: 16 }}>
          {"更新 Waffo 设置"}
        </Button>
      ...
      {/* 新增/编辑支付方式弹窗 */}
      <Modal
        title={editingPayMethodIndex === -1 ? "新增支付方式" : "编辑支付方式"}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            ...
            <Input
              ...
              placeholder={"例如：Credit Card"}
            />
            <Text type='tertiary' size='small'>{"用户在充值页面看到的支付方式名称，例如：Credit Card"}</Text>
          </div>
          <div>
            <div style={{ marginBottom: 4 }}>
              <Text strong>{"图标"}</Text>
            ...
            <div>
              <Text type='tertiary' size='small'>{"上传 PNG/JPG/SVG 图片，建议尺寸 ≤ 128×128px"}</Text>
            </div>
          </div>
          <div>
            <div style={{ marginBottom: 4 }}>
              <Text strong>{"Pay Method Type"}</Text>
            </div>
            <Input
              ...
              maxLength={64}
            />
            <Text type='tertiary' size='small'>{"Waffo API 参数，可空，例如：CREDITCARD,DEBITCARD（最多64位）"}</Text>
          </div>
          <div>
            <div style={{ marginBottom: 4 }}>
              <Text strong>{"Pay Method Name"}</Text>
            </div>
            <Input
              ...
              maxLength={64}
            />
            <Text type='tertiary' size='small'>{"Waffo API 参数，可空（最多64位）"}</Text>
          </div>
        </div>
      </Modal>
    </Spin>
