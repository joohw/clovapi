...
<script>
  ...
  import ChannelAddDialog from '$lib/components/admin/ChannelAddDialog.svelte';
  import ChannelEditDialog from '$lib/components/admin/ChannelEditDialog.svelte';
  import LogDetailDialog from '$lib/components/admin/LogDetailDialog.svelte';
  import {
    channelTypeParts,
    channelStatusParts,
    channelStatusClass,
  ...
  let showChannelAdd = false;
  let showChannelEdit = false;
  /** @type {number | null} */
  ...
  $: if (!showChannelEdit) editChannelId = null;

  /** @param {any} row */
  function openChannelEdit(row) {
    const id = row?.id;
    if (id == null || id === '') return;
    editChannelId = Number(id);
    ...
    <div class="admin-console-body">
      ...
      {:else if adminTab === 'channel'}
        ...
        <div class="mb-3 flex shrink-0 flex-wrap items-center justify-between gap-2">
          ...
          <Button size="sm" onclick={() => (showChannelAdd = true)}>添加渠道</Button>
        </div>
...
<ChannelAddDialog bind:open={showChannelAdd} onAdded={() => loadTabData('channel')} />
<ChannelEditDialog
  bind:open={showChannelEdit}
  channelId={editChannelId}
  onSaved={() => loadTabData('channel')}
/>
