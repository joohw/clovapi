...
<script>
  ...
  async function copyTokenKey(id) {
    ...
    await navigator.clipboard.writeText(`sk-${res.data.key}`);
  }
...
<div class="page-wrap">
  <div class="toolbar">
    <h1 class="text-2xl font-semibold">密钥</h1>
    <div class="flex items-center gap-2">
      <Button variant="outline" onclick={loadTokens} disabled={loading}>
        {loading ? '刷新中...' : '刷新'}
      </Button>
      <Button onclick={openCreate}>添加令牌</Button>
    </div>
  </div>
  ...
  <div class="panel">
    {#if loading}
      <div class="p-4 text-sm opacity-70">加载中...</div>
    {:else if tokens.length === 0}
      <div class="p-4 text-sm opacity-70">暂无令牌数据</div>
    {:else}
...
<Dialog.Root bind:open={showModal}>
  <Dialog.Content class="max-w-lg">
    ...
    <form class="space-y-3" onsubmit={submitForm}>
      <div>
        <label class="auth-label" for="token-name">名称</label>
        <Input
          id="token-name"
          type="text"
      ...
      <div>
        <label class="auth-label" for="token-quota">额度上限（美元，可选）</label>
        <Input
          id="token-quota"
          type="text"
          inputmode="decimal"
      ...
      <div>
        <span class="auth-label block mb-1">过期时间</span>
        <Select.Root type="single" bind:value={form.expired_time}>
          <Select.Trigger id="token-expire" class="w-full min-w-0 max-w-full">
