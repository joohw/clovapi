...
<script>
  ...
  let showWeChatBindModal = false;
  let showEmailBindModal = false;
  ...
  /** @type {'binding' | 'security' | 'notification' | 'pricing' | 'privacy' | 'sidebar'} */
  let dashboardTab = 'binding';
  ...
  async function bindWeChat() {
    ...
    const res = await apiPost('/api/oauth/wechat/bind', {
      code: inputs.wechat_verification_code,
    });
    if (res?.success) {
      showSuccess('微信账户绑定成功！');
      showWeChatBindModal = false;
      await getUserData();
    } else {
  ...
  async function bindEmail() {
    ...
    loading = true;
    const res = await apiPost('/api/oauth/email/bind', {
      email: inputs.email,
      code: inputs.email_verification_code,
    });
    if (res?.success) {
      showSuccess('邮箱账户绑定成功！');
      showEmailBindModal = false;
      if (userState?.user) {
...
<div class="page-wrap mt-2 w-full min-w-0">
    ...
    >
      <div class="border-b border-border p-4 md:p-6 md:pb-0">
        <div class="flex flex-wrap gap-1 border-b border-border pb-px">
          <button
            type="button"
            class="rounded-none px-3 py-1.5 text-sm {dashboardTab === 'binding'
              ? 'border-b-2 border-primary font-medium'
              : 'text-muted-foreground'}"
            onclick={() => (dashboardTab = 'binding')}
          >
            账户绑定
          </button>
          <button
      ...
      <div class="p-4 pt-4 md:p-6">
        {#if dashboardTab === 'binding'}
          <AccountManagement
            ...
            onOpenEmailBind={() => (showEmailBindModal = true)}
            onOpenWeChatBind={() => (showWeChatBindModal = true)}
            onOpenChangePassword={() => (showChangePasswordModal = true)}
            onOpenDeleteAccount={() => (showAccountDeleteModal = true)}
          />
        {:else if dashboardTab === 'security'}
          <AccountManagement
            section="security"
            {userState}
            {status}
...
<!-- 绑定邮箱 -->
<Dialog.Root bind:open={showEmailBindModal}>
  <Dialog.Content class="max-w-md">
    <Dialog.Header>
      <Dialog.Title>绑定邮箱地址</Dialog.Title>
    </Dialog.Header>
    <div class="space-y-3 py-2">
      ...
      <Input placeholder="验证码" bind:value={inputs.email_verification_code} />
      {#if turnstileEnabled && turnstileSiteKey}
        <TurnstileWidget sitekey={turnstileSiteKey} onVerify={(t) => (turnstileToken = t)} />
      {/if}
    </div>
    <Dialog.Footer class="gap-2">
      <Button variant="outline" onclick={() => (showEmailBindModal = false)}>取消</Button>
      <Button disabled={loading} onclick={bindEmail}>确定</Button>
    </Dialog.Footer>
  </Dialog.Content>
</Dialog.Root>

<!-- 微信 -->
<Dialog.Root bind:open={showWeChatBindModal}>
  <Dialog.Content class="max-w-md">
    <Dialog.Header>
      <Dialog.Title>绑定微信账户</Dialog.Title>
    </Dialog.Header>
    <div class="space-y-3 py-2 text-center">
      {#if status.wechat_qrcode}
        <img src={status.wechat_qrcode} alt="WeChat QR" class="mx-auto max-w-[200px]" />
      {/if}
      <p class="text-sm text-muted-foreground">
        微信扫码关注公众号，输入「验证码」获取验证码（三分钟内有效）
      </p>
      <Input placeholder="验证码" bind:value={inputs.wechat_verification_code} />
      <Button class="w-full" onclick={bindWeChat}>绑定</Button>
    </div>
  </Dialog.Content>
</Dialog.Root>
