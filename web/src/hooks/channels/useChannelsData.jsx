...
export const useChannelsData = () => {
  ...
  const upstreamUpdates = useChannelUpstreamUpdates({ refresh });
  ...
  const copySelectedChannel = async (record) => {
    try {
      ...
      if (res?.data?.success) {
        ...
        showError(res?.data?.message || "渠道复制失败");
      }
    } catch (error) {
      showError(
        "渠道复制失败: " +
  ...
  const batchSetChannelTag = async () => {
    ...
    if (res.data.success) {
      showSuccess(
        "已为 ${count} 个渠道设置标签！".replace('${count}', res.data.data),
      );
      await refresh();
  ...
  const batchDeleteChannels = async () => {
    ...
    const { success, message, data } = res.data;
    if (success) {
      showSuccess("已删除 ${data} 个通道！".replace('${data}', data));
      await refresh();
      setTimeout(() => {
  ...
  const deleteAllDisabledChannels = async () => {
    ...
    if (success) {
      showSuccess(
        "已删除所有禁用渠道，共计 ${data} 个".replace('${data}', data),
      );
      await refresh();
    } else {
      showError(message);
  ...
  const updateChannelBalance = async (record) => {
    ...
    if (success) {
      ...
      showInfo(
        "通道 ${name} 余额更新成功！".replace('${name}', record.name),
      );
    } else {
      showError(message);
    }
  ...
  const checkOllamaVersion = async (record) => {
    try {
      ...
      if (success) {
        ...
        const infoMessage = "当前 Ollama 版本为 ${version}".replace(
          '${version}',
          version,
        );
        ...
        showError(message || "获取 Ollama 版本失败");
      }
    } catch (error) {
      const errMsg =
  ...
  const testChannel = async (
    ...
    try {
      ...
      showError(`${"模型"} ${model}: ${error.message || "测试失败"}`);
    } finally {
      // 从正在测试的模型集合中移除
  ...
  const batchTestModels = async () => {
    ...
    try {
      showInfo(
        "开始批量测试 ${count} 个模型，已清空上次结果...".replace(
          '${count}',
          models.length,
        ),
      );
      ...
      showError("批量测试过程中发生错误: " + error.message);
    } finally {
