...
<script>
  ...
  import { renderQuota, usdToQuota, copy } from '$lib/dashboard/helpers.js';
  ...
  /**
   * 列表接口已返回脱敏 key，与复制时的 sk- 前缀保持一致便于辨认
   * @param {any} token
   */
  function maskedKeyLabel(token) {
    const k = token?.key;
  ...
  async function submitForm(event) {
    ...
    try {
      ...
      const res = await apiPost('/api/token/', payload);

      if (res?.success) {
        showModal = false;
        await loadTokens();
      } else {
        errorMsg = res?.message || '创建失败';
      }
    } catch (err) {
      errorMsg = '创建失败，请稍后重试';
    } finally {
...
>
  <div
    class="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between md:p-6 md:pb-4"
  >
  ...
  <div class="border-t border-border" role="presentation"></div>
        ...
        <TableBody>
          {#each tokens as token}
            <TableRow class={token.status !== 1 ? 'opacity-70' : ''}>
              <TableCell class="max-w-[min(100%,18rem)] font-mono text-xs whitespace-normal break-all text-muted-foreground">
              ...
              <TableCell class="text-right">
                <div class="flex flex-wrap items-center justify-end gap-1">
                  <Button variant="outline" size="xs" onclick={() => copyTokenKey(token.id)}>复制</Button>
...
<Dialog.Root bind:open={showModal}>
  <Dialog.Content class="max-w-lg">
    <Dialog.Header>
      <Dialog.Title>创建密钥</Dialog.Title>
    </Dialog.Header>
